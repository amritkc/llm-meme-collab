...
// Fixed template for deterministic runs (Disaster_Girl)
const FIXED_TEMPLATE = {
  templateId: '03',
  name: 'Disaster_Girl.jpg',
  path: '../src/assets/templates/daily/Disaster_Girl.jpg',
  description: "Girl smiling while chaos burns behind her.",
};
...
async function testSelectTemplate() {
  ...
  console.log('📋 Task: AI selects the best meme template for "Everyday Struggles"');
  console.log('📦 Testing with 4 templates (Everyday Struggles)\n');
  ...
  try {
    ...
    const templates = [
      ...
      },
      {
        ...
        name: 'Disaster_Girl.jpg',
        path: '../src/assets/templates/daily/Disaster_Girl.jpg',
        description: "Girl smiling while a house is burning behind her.",
      },
    ];
...
async function testGenerateCaptions(selectedTemplate, runLabel) {
  try {
    ...
    const requestBody = {
      ...
      descriptionOfMemeTemplate: selectedTemplate?.description || 'Disaster Girl template',
    };      
...
async function runAllTests() {
  ...
  // Run ai-generate-captions 6 times using Disaster_Girl (Test 1 disabled)
  const runResults = [];
  for (let i = 1; i <= 6; i++) {
