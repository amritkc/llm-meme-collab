...
// Fixed template for deterministic runs (One_Does_Not_Simply)
...
async function testGenerateCaptions(selectedTemplate, runLabel) {
  try {
    const templatePath = selectedTemplate?.path || '../src/assets/templates/tech/Change_My_Mind.jpg';
    ...
    const requestBody = {
      ...
      descriptionOfMemeTemplate: selectedTemplate?.description || 'Change My Mind template',
    };
...
async function runAllTests() {
  ...
  // Run ai-generate-captions 4 times using One_Does_Not_Simply (Test 1 disabled)
  const runResults = [];
  for (let i = 1; i <= 4; i++) {
